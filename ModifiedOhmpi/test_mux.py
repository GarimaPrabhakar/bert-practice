...
if a== '3': 
    with open('scheme3d.ohm') as f:
        lines = f.readlines()

    start = lines.index('# a b m n r\n')
    scheme = lines[start:]
    elecs = [int(scheme[i][0]) for i in range(1, len(scheme))]

    print("STARTING DIPOLE DIPOLE SCHEME FOR ELECTRODE A")
