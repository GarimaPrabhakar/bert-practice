...
class OhmPi(object):
    ...
    def run_measurement_time_domain(self, quad=None, nb_stack=None, injection_duration=None,
                        autogain=True, strategy='constant', tx_volt=5, best_tx_injtime=0.1,
                        cmd_id=None):
        """Measures on a quadrupole and returns transfer resistance.
        ...
        if self.on_pi:
            ...
            if not out_of_range:  # we found a Vab in the range so we measure
                ...
                for n in range(0, nb_stack * 2):  # for each half-cycles
                    ...
                    # truncate the meas array if we didn't fill the last samples
                    # meas = meas[:k + 1] MODIFIED: SAVE CHARGING AND DISCHARGING CURVE
            ...
            d = {
                ...
                "fulldata": fulldata, # is this comma supposed to be here?
            }
    ...
    def run_sequence_time_domain(self, cmd_id=None, **kwargs):
        ...
        for i in range(0, n):
            ...
            # run a measurement
            if self.on_pi:
                acquired_data = self.run_measurement_time_domain(quad, **kwargs)
            else:  # for testing, generate random data
                acquired_data = {
